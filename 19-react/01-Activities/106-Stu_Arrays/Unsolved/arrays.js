...
const filterPrincess = princesses.filter(function(princess) {
  return princess.name.startsWith("M");
});

console.log("princess start with M ", filterPrincess)
...
const ageTotal = princesses.reduce(function(sum, princess) {
...
const sum = princesses.reduce(function(total, princess) {
  return total + princess.age;
}, 0);

const quantity = princesses.length;
const avg = sum/quaantity;
